...
print(p<-matrix(data=c(rep(1/2,3),rep(4/10,3),rep(1/10,3)),nrow=3,ncol=3))
print(priors<- apply(p*h,1,sum))
@
\item %(B)
<<>>=
print(h_obs <- matrix(data=c(h['a',], 1-h['b',], h['c',]),nrow=3,ncol=3,dimnames=list(c("a","b","c"),c("h1","h2","h3")),byrow=TRUE))
apply(h_obs,2,prod)
@
...
print(mean.gpa.by.class <- daply(dat,.(class),colwise(mean, .(mean.GPA = GPA))) )
print(sd.gpa.by.class <- daply(dat,.(class),colwise(sd, .(sd.GPA = GPA))))
n.n <- sum(dat$class == "N") #sums the vector of (TRUE, FALSE...)
n.h <- sum(dat$class == "H")
pc.n <- n.n / (n.n+n.h)
pc.h <- n.h / (n.n + n.h)
# example of grabbing 'H' class GPA column
#sd.gpa.by.class$GPA[ sd.gpa.by.class$class == "H"]
@
