...
After seeing all the negative examples, the algorithm must make another pass and
inspect all the positive examples.
If there is a positive instance $x$ such that all the boolean assignments of $x$
are contained in $I$, then there is no way to consistently predict $x$ and all
the negative instances simultaneously.
In this case, and only in this case, does the most consistent hypothesis \emph{not}
exist (which is different from the empty disjuction hypothesis).
Otherwise, there is every positive instance $x$ has at least one boolean assignment not in $I$.
Or, if we defined the set $\mathcal{H}$ to be the set of all possible attribute assignments,
we can say that every positive instance $x$ has at least one boolean assignment in
$C = \mathcal{H} \setminus I$.
Due to the definition of $\mathcal{H}$ and the construction of $I$, the set
$C$ when viewed as a disjunction of its elements represents the most general consistent hypothesis.
...
Prunes all boolean assignments in negative trainging instances from the hypothesis.
...
If there is no consistent hypothesis, then the algorithms triggers an ``EXPLODE''
subroutine which signifies an error.
After the algorithm runs, the disjunction $h$ is the most general consistent hypothesis.
}
\label{Alg:prune}
\begin{algorithmic}
\STATE Initialize $I$ as an empty set.
\FORALL {$(x,y) \in \textnormal{TrainingSet}$}
	\IF {$y = -1$}
		\FORALL {$i=1,\dots,n$}
			\STATE Add $(a_i = x_i)$ to $I$.
		\ENDFOR
	\ENDIF
\ENDFOR
\FORALL {$(x,y) \in \textnormal{TrainingSet}$}
    \IF {$y = +1 \textbf{ and } x_i \in I \textbf{ for all } i\in\left\{1,\dots,n\right\}$}
        \STATE EXPLODE!
	\ENDIF
\ENDFOR
\STATE $\mathcal{H} \gets \left\{(a_k = b) : 1 \leq k \leq n, b \in \left\{0,1\right\}\right\}$
\STATE $C \gets \mathcal{H} \setminus I$
\STATE $h \gets \displaystyle\bigvee_{(a_k=b) \in C} (a_k=b)$
\end{algorithmic}
\end{algorithm}
